 ...
 <div class="relative w-full mx-auto overflow-hidden">
  ...
  <div class="flex transition-transform duration-500" id="slider">
      ...
      <div class="w-full flex-shrink-0 relative h-64 md:h-80 lg:h-96 xl:h-112">
        <!-- Set height here -->
        <img src="{{ asset('storage/'.$banner->image) }}" class="w-full h-full object-cover" alt="Slide 1">
...
<h2 class="text-4xl font-bold text-cyan-600 flex justify-center lg:mt-14 m-6">{{$about->title}}</h2>
<section class="about-us container mx-auto px-4 lg:mt-20 bg-cover bg-center relative" style="background-image: url('{{ asset('storage/'.$about->image) }}');">
      ...
      <div class="about-image md:w-1/2 rounded-lg overflow-hidden shadow-lg transform transition duration-500 hover:scale-105 hover:shadow-2xl">
        <div class="relative">
          ...
          <div class="absolute bottom-0 left-0 right-0 p-4 text-gray-100 text-center transform translate-y-full transition-transform duration-300 ease-in-out hover:translate-y-0">
            <h3 class="text-lg font-semibold text-white">{{$about->description}}</h3>
            <p class="text-sm text-white mb-2">{{$about->description}}</p>
          </div>
        </div>
      </div>
...
<article class="cow-tradition px-4 md:px-16 lg:mt-28 m-6">
  <h1 class="text-3xl font-bold text-cyan-600 text-center mb-4">{{ $heading->title }}</h1>
  <p class="text-center mb-4 text-lg">{{ $heading->sub_title }}</p>
  <p class="text-center mb-6 text-xl italic font-bold">"{{ $heading->qoutes }}"</p>

  <div class="flex flex-col md:flex-row gap-8">
    <!-- Text Content -->
    <div class="w-full md:w-1/2 p-4">
      @foreach ($sadhgurus as $sadhguru)
      <h2 class="text-2xl font-semibold mb-4 text-cyan-700">{{ $sadhguru->title }}</h2>
      <p class="mb-4 text-lg leading-relaxed">{{ $sadhguru->para1 }}</p>
      <p class="mb-4 text-lg leading-relaxed">{{ $sadhguru->para2 }}</p>
      <blockquote class="border-l-4 border-cyan-600 pl-4 italic text-lg leading-relaxed mb-4">
        "{{ $sadhguru->qoutes }}"
        <footer class="text-right text-cyan-600 font-semibold mt-2">{{ $sadhguru->auther }}</footer>
      </blockquote>
    </div>
    ...
    <div class="w-full md:w-1/2 p-4">
      <img 
        src="{{ asset('storage/'.$sadhguru->image) }}" 
...
<h1 class="text-3xl font-bold text-cyan-600 text-center lg:mt-28 m-6">{{$container->title}}</h1>
  ...
  <div class="flex flex-col md:flex-row gap-8">
    ...
    <div class="w-full md:w-1/2 p-4 relative">
      <div x-data="{ currentSlide: 0, interval: null }" 
           ...
           <div class="relative h-64 md:h-80 lg:h-96 overflow-hidden rounded-lg shadow-lg">
        <!-- Images -->
        <img src="{{ asset('storage/'.$container->image1) }}" 
    ...
    <!-- Ritual Details -->
    <div class="w-full md:w-1/2 p-4">
      <h2 class="text-2xl font-semibold text-cyan-700 mb-4">{{$container->sub_title}}</h2>
      <p class="text-lg mb-4 leading-relaxed text-gray-700">
       {{$container->paragraph}}
      </p>
      {{-- <p class="text-lg mb-4 leading-relaxed text-gray-700">
        You can make your offering in person on a date of your choosing and attend the Gau Pooja at Linga Bhairavi if you wish.
      </p>
      <p class="text-lg leading-relaxed text-gray-700">
        For those unable to visit personally, you may provide the names and nakshatras (birth stars) of up to four family members. These details will be written on a palm leaf and placed at Devi’s feet for blessings. The offering will be made on their behalf, and prasadam can be sent upon request within India.
      </p> --}}
    </div>
  </div>
</article>
...
<section class="relative w-full min-h-screen bg-black mt-28">
  <!-- Background Image -->
  <img 
    src="{{ asset('storage/'.$video->background_image) }}" 
  ...
  <div class="relative z-10 flex flex-col items-center justify-center min-h-screen p-4 sm:p-6 md:p-8 lg:p-12 overflow-y-auto">
    ...
    <h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold mb-6 text-center text-cyan-200">
      <span class="border-b-2 border-cyan-200 pb-2">{{$video->title}}</span>
    </h1>
...
<section class="relative w-full px-4 sm:px-8 md:px-12 lg:px-16 mt-12">
  ...
  <div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-2 gap-8">
    ...
    <div class="relative overflow-hidden rounded-lg shadow-lg group">
      <img src="{{ asset('storage/'.$gallery->image) }}" alt="Cow image 3" class="w-full h-full object-cover transition-transform duration-300 ease-in-out group-hover:scale-110" loading="lazy">
...
<section class="container mx-auto px-4 py-12 mt-8">
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
    ...
    <div class="bg-gray-200 p-6 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
      <h2 class="text-2xl font-semibold text-gray-800 mb-4">{{$card->title}}</h2>
...
<section class="bg-white shadow-lg rounded-lg mt-12 lg:p-8">
  ...
  <div class="container mx-auto px-4 py-8">
    <h1 class="text-3xl font-bold mb-8 text-center text-cyan-700">{{$story->title}}</h1>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
      ...
      <div class="flex flex-col items-center text-center bg-gray-100 p-6 rounded-lg shadow-md transition-transform duration-300 hover:scale-105">
        <img src="{{ asset('storage/'.$story->image) }}" alt="Story 1" class="w-full h-48 object-cover rounded-lg mb-4">
